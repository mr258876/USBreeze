...
// volatile uint16_t RGB_WS2812_Buffer[24 * 16];
volatile uint8_t RGB_Lamp_Colors[RGB_LAMP_COUNT * RGB_CHANNELS_PER_LAMP];
...
void RGB_Control_thread(const void *dummy)
{
    ...
    while (1)
    {
        ...
        {
            RGB_Control_Show_RGB_Blocking_From_Array(RGB_Lamp_Colors, RGB_LAMP_COUNT);
        }
    }
}

void RGB_Control_Set_Autonomous_Mode(int autonomous_on) { RGB_Autonomous_Mode = autonomous_on; }
uint8_t RGB_Control_get_Autonomous_Mode(void) { return RGB_Autonomous_Mode; }
